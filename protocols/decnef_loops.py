...
def minimal_loop(train_loader, generator, discriminator, target_class, lambda_, n_iter, device,  
    ...
    if plot:
        ...
        for ax in axx[n_iter:]:
            ax.axis('off')
    # fig_gen, axxx = plt.subplots()
    # axxx=show_images_grid(generated_images,np.floor(np.sqrt(n_iter+1)).astype(int),axxx, title=title, title_color = title_color, title_fontsize = 26)
...
def show_trajectories(vae_2d, discriminator, train_loader, target_class, class_names, update_rule_func, p_scale_func, filename, extension='png',
    ...
    for iter_ in range(n_traj):
        ...
        generated_images[iter_], trajectory, Dnorm[iter_], probabilities[iter_], all_probabilities[iter_], sigma[iter_]  = minimal_loop(train_loader, vae_2d, discriminator, target_class, lambda_, n_iter, device,
                                                                                                                                        class_names, classes_mean,update_rule_func, p_scale_func, z_current=z_current, 
                                                                                                                                        random_state=(iter_+1)*random_state, ignore_discriminator=ignore_discriminator,
                                                                                                                                        title=f'Trajectory {iter_+1}', title_color = colors[iter_], plot=False, 
                                                                                                                                        **f_p_kwargs)
    ...
    fig_traj1, axs1, fig_traj2, axs2, colors = plot_vae2d_trajectory(vae_2d, trajectories, train_loader, target_class, class_names, 
                                                  figsize_gaussians, zoom_radius = zoom_radius, mark_origin= start_from_origin)
    # axs1.axis('off')
    axs2.axis('off')
    ...
    # traj_img = fig_to_image(fig_traj)
    
    # for i in range(n_traj):
    #     print('TRAJ ',i)
    #     print(probabilities[i][:10])
    #     print( probabilities[i][-10:])
    #     print('-------------------')
    ...
    # ax_41 = fig3.add_subplot(gs[4, 0])
    # ax_42 = fig3.add_subplot(gs[4, 1])
    # ax_43 = fig3.add_subplot(gs[4, 2])
    # ax_44 = fig3.add_subplot(gs[4, 3])
    # fig_gauss, ax_gauss = plt.subplots()
    # ax_gauss.imshow(traj_img)
    # ax_gauss.axis('off')
    
    row2 = [ax_1, ax_2, ax_3, ax_4]
    row3 = [ax_21, ax_22, ax_23, ax_24]
    row5 = [ax_31, ax_32, ax_33, ax_34]
    # row5 = [ax_41, ax_42, ax_43, ax_44]
    
    # for a in row3:
    #     for spine in a.spines.values():
    #         spine.set_visible(True)  # Ensure spines are not hidden
    axtitle= 'logit(p)' if logit else 'Probability'
    
    xticks = {}
    for idx in idxs:
        length = len(Dnorm[idx])
        xticks[idx] = np.floor(np.linspace(0, length, 5)).astype(int) if n_iter >= 5 else range(length+1)
    for i, a in enumerate(row5):
        
        # a.plot(Dnorm[idxs[i]])
        a.plot(Dnorm[idxs[i]])
        a.axhline(y=Dnorm[idxs[i]][0], color='grey', ls='--')
        a.set_title('Distance', color = colors[idxs[i]], fontsize = 38)
        a.set_xlabel('Iterations', color = colors[idxs[i]], fontsize = 36)
        # if logit: a.set_yscale('logit')
        a.set_xticks(xticks[idxs[i]])
        a.set_xticklabels(xticks[idxs[i]], fontsize = 28)
        for spine in a.spines.values():
                spine.set_visible(True)  # Ensure spines are not hidden
    
    for i, a in enumerate(row3):
        
        a.plot(probabilities[idxs[i]], color ='tab:red', label='Probability')
        ...
        ax2.plot(sigma[idxs[i]], label = 'sigma')
        # a.set_title(f'p_start={probabilities[idxs[i]][0]:.3f}, p_end={probabilities[idxs[i]][-1]:.3f}', color = colors[idxs[i]], fontsize = 20)
        a.set_xlabel('Iterations', color = colors[idxs[i]], fontsize = 30)
        a.axis('tight')
        a.set_xticks(xticks[idxs[i]])
        a.set_xticklabels(xticks[idxs[i]], fontsize = 28)
    ...
    ax_34.set_xlabel('Iterations', color = colors[idxs[3]], fontsize = 30)
    ax_34.set_xticks(xticks[idxs[i]])
    ax_34.set_xticklabels(xticks[idxs[i]], fontsize = 28)
    ...
    # ax_34.legend()
    # ax_24 = fig3.add_subplot(gs[2, 3])  # Re-add it
    # # a.plot(probabilities[idxs[3]])
    # ax_24.set_title(axtitle, color = colors[idxs[3]], fontsize = 24)
    # ax_24.set_xlabel('Iterations', color = colors[idxs[3]], fontsize = 18)
    # ax_24.set_xticks(xticks)
    # ax_24.set_xticklabels(xticks)
    # ax_24.set_frame_on(True)
    # ax_24.set_visible(True)  # Ensures the axis is visible
    # ax_24.xaxis.set_visible(True)
    # ax_24.yaxis.set_visible(True)
    # ax_24.spines['top'].set_visible(True)
    # ax_24.spines['right'].set_visible(True)
    # ax_24.spines['bottom'].set_visible(True)
    # ax_24.spines['left'].set_visible(True)
    ...
    for i, a in enumerate(row2):
        length = len(Dnorm[idxs[i]])
        if length >= 50: 
            chosen = np.floor(np.linspace(0, length, 49)).astype(int)
            imgs = [generated_images[idxs[i]][idx] for idx in chosen]
            length = 49
        else: 
            imgs = generated_images[idxs[i]]
        ...
        if length==49:
            ...
            aa.tick_params(axis='y', labelcolor=colors[idxs[i]])
            ...
            a.tick_params(axis='y', labelcolor=colors[idxs[i]])
            a.set_yticklabels(chosen[6::7][::-1], fontsize = 36)
            a.set_frame_on(False)
            a.get_xaxis().set_visible(False)
        a.set_title(f'Trajectory {idxs[i]+1}', color = colors[idxs[i]], fontsize = 38)
    ...
    # fff, aaa = plt.subplots(4)
    # for aa, v in zip(aaa, all_probabilities.values()):
    #     aa.plot(v)
...
"""
#%%
def show_many_trajectories(vae_2d, discriminator, train_loader, target_class, combo_names, class_names, update_rule_func, p_scale_func, filename, extension='png',
                      n_iter = 15, n_traj=10, lambda_ = 0.15, device='cuda', ignore_discriminator=0, title = '', figsize = 14, figsize_gaussians= 16,
                      logit=False, random_state=0, zoom_radius = 1.5, start_from_origin=False,
                      # starting_zs=None,
                      **f_p_kwargs):
    latents_mean, latents_stdvar, labels = get_data_predictions(vae_2d, train_loader)
    ...
    for iter_ in range(n_traj):
        z_current = None if start_from_origin else torch.normal(0.,1.,(1, 2))#starting_zs[iter_]
        generated_images[iter_], trajectory, Dnorm[iter_], probabilities[iter_], all_probabilities[iter_], sigma[iter_]  =  minimal_loop(train_loader, vae_2d, discriminator, target_class, lambda_, n_iter, device,
                                                                                                                                        combo_names, classes_mean,update_rule_func, p_scale_func, z_current=z_current, 
                                                                                                                                        random_state=iter_, ignore_discriminator=ignore_discriminator,
                                                                                                                                        plot=False, 
                                                                                                                                        **f_p_kwargs)
        trajectories.append(trajectory)
    ...
    xticks = {}
    length = len(Dnorm[0])
    xticks= np.floor(np.linspace(0, length, 5)).astype(int) if n_iter >= 5 else range(length+1)
    
    for i in range(n_traj):
        ax_2.plot(Dnorm[i])
    ax_2.axhline(y=Dnorm[0][0], color='grey', ls='--')
    ax_2.set_title('Distance', fontsize = 38)
    ax_2.set_xlabel('Iterations', fontsize = 36)
    # if logit: ax_2.set_yscale('logit')
    ax_2.set_xticks(xticks)
    ax_2.set_xticklabels(xticks, fontsize = 28)
    for spine in ax_2.spines.values():
            spine.set_visible(True)  # Ensure spines are not hidden
 
    ax_1.plot(probabilities[i], color ='tab:red', label='Probability')
    ax_1.set_ylabel('Probability', color='tab:red', fontsize = 28)
    ax_1.tick_params(axis='y', labelcolor='tab:red', size = 28, labelsize = 28)

    # ax_1.set_title(axtitle, color = colors[idxs[i]], fontsize = 24)
    # ax_1.set_xlabel('Iterations', color = colors[idxs[i]], fontsize = 18)
    # if logit: ax_1.set_yscale('logit')
    ax2 = ax_1.twinx()
    ax2.set_ylabel('Variance', color='tab:blue', fontsize = 30)
    ax2.tick_params(axis='y', labelcolor='tab:blue', size = 28, labelsize = 28)
    for i in range(n_traj):
        ax2.plot(sigma[i], label = 'sigma')
    # ax_1.set_title(f'p_start={probabilities[idxs[i]][0]:.3f}, p_end={probabilities[idxs[i]][-1]:.3f}', color = colors[idxs[i]], fontsize = 20)
    ax_1.set_xlabel('Iterations', fontsize = 30)
    ax_1.axis('tight')
    ax_1.set_xticks(xticks)
    ax_1.set_xticklabels(xticks, fontsize = 28)
    ax_1.tick_params(axis='y', size = 28, labelsize = 28)
